...
public class BFS
{
    ...
    {
        //不在查找的节点 且 对应地图格子的类型不是障碍物 才能加入查找字典
    ...
    public List<Point> FindMinPath(ModelBase model, int step, int endRowIndex, int endColIndex)
    {
        ...
        else
        {
            ...
            int min_dis = Mathf.Abs(minPoint.RowIndex - endRowIndex) +Mathf.Abs(minPoint.ColIndex - endColIndex);//计算距离
